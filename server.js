...
var gameloop = (() => {
    ...
    let collide = (() => {
        ...
        return collision => {
            ...
            //handle portals
            if ((instance.type === "tank" && other.isPortal && instance.isPortal !== other.isPortal) ||
               (other.type === "tank" && instance.isPortal && instance.isPortal !== other.isPortal)) {
              if (instance.isPortal) {
                if (other.skill.level >= 0) {
                  //if (other.team == -1 || instance.team == -2) {
                    let o = room.randomType("norm");
                    other.x = o['x'];
                    other.y = o['y'];
                    other.team = -9
                    other.define(Class.dreadnought2);
                    other.skill.set(Array(10).fill(0))
                    other.skill.points = 60
                  //}
                }
              } else if (other.isPortal) {
                if (instance.skill.level >= 10) {
                  //if (instance.team == -1 || instance.team == -2) {
                    let o = room.randomType("norm");
                    instance.x = o['x'];
                    instance.y = o['y'];
                    instance.team = -9
                    instance.define(Class.dreadnought2);
                    instance.skill.set(Array(10).fill(0))
                    instance.skill.points = 60
                  //}
                }
              }
            }
...
let server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    case '/':
      res.writeHead(200)
      res.end(`<!DOCTYPE html><h3>Arras</h3><button onclick="location.href = 'http://arras.cx/#host=' + location.host">Open</button>`)
    break
    case '/mockups.json':
...
// make walls
function makePermanentWalls(loc) {
  let o = new Entity(loc);
  ...
  o.protect();
  o.onDeath = () => {
    makePermanentWalls(loc);
  };
}

for (let loc of room["wall"]) makePermanentWalls(loc)
